...
use glfw::{Context, Key, Action, WindowMode, WindowHint};
...
impl Tetris
{
    pub fn new(screen_width: u32, screen_height: u32, title: &str) -> Self
    {
        ...
        let (mut window, events) = glfw.create_window(screen_width, screen_height, title, WindowMode::Windowed)
    ...
    pub fn run(&mut self)
    {
        ...
        while !self.window.should_close()
        {
            ...
            gl_call!(gl::ClearColor(135.0 / 255.0, 206.0 / 255.0, 235.0 / 255.0, 1.0));
            ...
            {                
                ...
                let model = glm::rotate(&model, x * sector_angle - PI * 0.5, &glm::vec3(0.0, 1.0, 0.0));

                let model = glm::translate(&model, &glm::vec3(0.0, pos_y, 0.0));

                let final_mat = projection * self.camera.view * model;
    
                gl_call!(gl::UniformMatrix4fv(u_final_mat, 1, gl::FALSE, final_mat.as_ptr()));
        ...
        for (_, event) in glfw::flush_messages(&self.events)
        {
            match event
            {
                ...
                WindowEvent::Key(key, _, action, _) => 
                {
                    ...
                    if action == Action::Release { break; }
                    ...
                    match key
                    {
                        ...
                        Key::F1     =>
                        {
                            self.window.set_cursor_mode(
                                if self.window.get_cursor_mode() == CursorMode::Disabled { CursorMode::Normal }
                                else { CursorMode::Disabled }
                            );
                ...
                WindowEvent::CursorPos(x, y) if self.cursor_disabled => self.camera.look_at(glm::vec2(x as f32, y as f32)),
                _ => {}
            }   
        }
    }
