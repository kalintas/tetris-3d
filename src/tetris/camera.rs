...
impl Camera
{
    ...
    {
        self.handle_key(window, Key::D,  glm::normalize(&glm::cross(&self.direction, &self.up)));
        self.handle_key(window, Key::A, -glm::normalize(&glm::cross(&self.direction, &self.up)));
    ...
    pub fn look_at(&mut self, pos: glm::Vec2)
    {
        ...
        let direction = glm::vec3(self.yaw.cos() * pitch_cos, self.pitch.sin(), self.yaw.sin() * pitch_cos);
